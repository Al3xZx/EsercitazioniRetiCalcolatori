package reti.src.it.unical.dimes.traccia2;
...
class MulticastListener extends Thread {
	...
	int tcpPort; // � la porta su cui il Communicator � in ascolto per ricevere
					// msg TCP
					// (questo valore verr� inserito nel msg di risposta)
	...
	public void run() {
		try {
			...
			while (true) {
				...
				// (remTCPPort),
				// cio� la porta su cui il mittente � in ascolto per ricevere
				// msg TCP
				byte[] buf = new byte[50];
				...
				// era stato inviato da un altro Communicator (non da s�
				// stesso);
...
class SocketListener extends Thread {
		...
		public void run() {
			try {
				...
				while (true) {
					// Ricevo messaggio di risposta via TCP (� la risposta di un
					// altro communicator al msg
					...
					// che ha risposto
					// e che viene usata per ricevere (a sua volta) msg TCP
					// (remTCPPort)
